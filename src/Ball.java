...
class Ball {
  private Sketch s;
  private float diameter; // Ball's diameter
  private float x; // Ball's center x
  private float y; // Ball's center y
  private int col; // Ball's color
  private float speedY; // Ball's speed for x and y
  ...
  public Ball(Sketch sketch) {
     ...
     */
     ...
     speedY = s.random(-10,5);

  }

  /** This constructor lets you specify all of the ball instance variables */
  public Ball(Sketch s, float X, float Y, float ballDiam, int ballColor, float sx, float sy, int bordColor, int fillCol) {
